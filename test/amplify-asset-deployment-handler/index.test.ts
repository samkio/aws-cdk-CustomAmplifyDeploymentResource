...
import {
  onEvent,
  isComplete,
} from "../../lib/amplify-asset-deployment-handler";
...
describe("handler", () => {
  ...
  it("onEvent CREATE pending job", async () => {
    ...
    try {
      ...
      // THEN
      expect(e).toEqual("Amplify job already running. Aborting deployment.");
  ...
  it("isComplete CREATE no JobId", async () => {
    ...
    try {
      ...
      expect(e.message).toEqual(
        "Unable to determine Amplify job status without job id"
      );
  ...
  it("onEvent unsupported resource type", async () => {
    ...
    try {
      ...
      // THEN
      expect(e.message).toEqual(
        'Unsupported resource type "Custom::BadResourceType"'
      );
      expect(getJobRequest).not.toHaveBeenCalled();
      expect(getJobResponse).not.toHaveBeenCalled();
    }
